...
## 2026-09-21 消息间隔：统一滑杆 + 逐条设置

### 问题：三档之间跨度太大，而「个别几条例外」根本没地方表达

节奏原本是三个档：快 0.8 秒 / 标准 1.5 秒 / 慢 2.5 秒。两个问题：

1. 0.8 与 1.5 之间差着近一倍，想做「1 秒上下」的效果没有地方可选；
2. 有些对话就是靠**某几处**的停顿出效果——对方隔了几秒才回、同一人连着发两条，
   三档只能整段一起变，表达不了。

### 模型：一份设置里放两种设法

`chat-playback.ts` 的 `PlaybackPace`（`'fast' | 'normal' | 'slow'`）换成一份 `PaceSetting`：

```ts
interface PaceSetting {
  mode: 'uniform' | 'perMessage'
  paceMs: number        // 统一间隔，0.5–3 秒（100ms 一格）
  gaps: number[]        // 逐条间隔，0.2–10 秒；gaps[i] 是「第 i+1 条出现前等多久」
}
```

- **两个值都留着**，切模式不丢对方的设置。逐条模式里的统一间隔兼作「新消息的默认间隔」：
  导入更长的对话后，多出来的那些间隔按它出现，改起来才有起点。
- **逐条的范围比统一宽**（0.2–10 秒 vs 0.5–3 秒）。这一档的意义就是「大体上按统一来、
  个别几条例外」，例外得真有地方可去：连发要能压到 0.2 秒，长停顿要能拉到 10 秒。
- **N 条消息 = N-1 处间隔**：首条消息前面是开场静置（`playbackLeadInMs`，1.2 秒），
  它不属于「两条消息之间」，所以不占一行，逐条模式也没法改它（改它等于改所有对话的开头）。

时间轴只改了一处：`buildPlaybackTimeline` 里那个 `atMs += paceMs` 变成
`atMs += gaps ? gaps[index - 1] : paceMs`。首条仍然落在 `leadInMs`（`index > 0` 才加间隔），
`totalMs` 仍然是「最后一条的 atMs + tailMs」。**预览播放与导出视频还是同一条时间轴**，
所以逐条调完导出的就是看到的那一版——这条约束没有变。

### 清洗：`normalizeMessageGaps(value, count, fallbackMs)`

界面与时间轴都先过它，返回长度恒为 `count - 1`：

- 多出来的**截掉**（那些消息已经不在对话里了）；
- 缺的用 `fallbackMs`（就是统一间隔）**补齐**——新加进来的消息不至于没有起点；
- 每一项都过 `clampPaceGapMs`（0.2–10 秒、100ms 一格），非数字或 `NaN` 这类脏值同样退回
  `fallbackMs`（而不是全局默认值 1500，否则「缺项按统一间隔」这条规则会自相矛盾）。

`clampPaceMs(NaN)` / `clampPaceGapMs(NaN)` 退回**默认值**而不是 0，理由与 `clampVideoSidePad`
一致：这里给的是「隔多久」，退回 0 会让消息挤成一团。

### 界面：`src/components/PaceOption.tsx`

一个控件同时出现在**导出弹窗**（`VideoExportDialog` 的「消息出现节奏」）和**预览上方的播放条**
（`ChatPlaybackBar` 折叠面板里的第一项），与 `SidePadOption` 一样是「同一份值、两处入口」。

- 上面是模式切换（统一间隔 / 逐条设置），下面一条滑杆；
- 逐条模式下追加一个列表：每行 = 序号 + 该条消息的摘要 + 滑杆 + 秒数，
  列表 `max-height` 滚动，行数多了也不会把面板撑开（队列最多 120 条，即最多 119 行）；
- 「全部设为 X 秒」一键把当前统一值铺到所有间隔上（先铺一遍再改个别几处，比一条条拖快）；
- `uniformOnly` 给批量页用——**批量页只给统一间隔**：逐条间隔是按「第几条」排的，
  而每一组是另一段对话、条数也不一样，套过去只会让每一组都莫名其妙。所以
  `BatchVideoPlan` / `BatchVideoChoice` 里只有 `paceMs`，没有 `gaps`；弹窗里会写一句说明。

行上的消息摘要走 `messageGapLabel(msg, sender)`：文字直接用内容（超 16 字截断），
图片 / 语音 / 红包 / 转账写成 `[图片]` 这类词——看不到内容的消息也得认得出是哪一条。

### 偏好与老数据

- `PlaybackPrefs.pace` 从字符串档位变成 `PaceSetting` 对象，`canonical()` 里逐项比较
  （`mode` / `paceMs` / `gaps.join('|')`），键序不同仍然算同一份。
- **旧值自动折算**：`paceMsFromLegacy()` 把 `'fast' | 'normal' | 'slow'` 读成 800 / 1500 / 2500 毫秒，
  老用户不会被这次改版打回默认节奏。旧偏好没有的字段照常逐项兜底。
- `defaultWorkspacePrefs()` 里 `pace` 单独给一份新对象（`gaps` 是数组，不能和默认值共享引用）。

### 单测

`chat-playback.test.ts`（新增 6 条、改写 1 条）：两个范围的夹取与对齐、「NaN 退回默认值而非 0」、
逐条模式的时间轴（首条仍是开场静置）、统一模式不看逐条间隔、逐条模式缺的补齐 / 多的截掉、
只有一条消息时没有间隔、`normalizeMessageGaps` 的四种形状、`normalizePaceSetting` 逐项清洗、
旧的档位折算、摘要文案、行上的消息摘要。

`batch-video.test.ts`（新增 1 条）：整批只认统一间隔、落到录制方案里再夹一次、
时间轴确实是「每条之间都是同一个值」。

`workspace-prefs.test.ts`：脏数据里 `pace: 'zoom'` 仍退回默认；新增「旧的档位折算成毫秒、
逐条间隔越界夹回、脏值逐项兜底」一条；`canonical()` 的改写覆盖「改统一间隔」「改逐条列表」
「逐条列表内容一样就算同一份」三种情形；默认值那条额外确认 `gaps` 是新的空数组。

### 验证边界

纯逻辑由单测兜底，本机**没有浏览器**：滑杆的手感、逐条列表在窄面板里的排版、
一百多行滑杆的滚动性能，需要在 Chrome / Edge 里点一遍确认。

## 2026-09-22 头像：换对话时随机补 + 从用过的头像里直接选

### 一、换了一段新对话才随机补头像

**问题**：头像原来只有「按名字续上」这一条路（`carryOverAvatars` + `applyPresetsToUsers`）。
角色名一换，全部落回解析器的灰色默认头像，用户得一个个点着换——而他素材库里明明攒着几十张。

**做法**：`src/lib/user-avatars.ts` 新增两个函数。

- `sameRoster(previous, next)`：两次解析出来的**角色名单**是否一模一样。归一化名字
  （`avatarNameKey`：忽略首尾空白、内部连续空白与大小写）、**不看顺序**、先比长度。
  判断依据是名字而不是 id —— id 每次解析都从 1 重排，按 id 比对等于永远「变了」。
- `drawAvatars(users, pool, { random })`：给**还没有头像**的角色随机抽一张。三条规则：
  1. 已经有头像的一位都不动——续上的旧头像、用户自己指定的，优先级都在随机之上；
  2. 同一次里尽量不重复：池子够分就人人不同，不够分才转第二圈；
  3. **已经有人用着的先让开**：`taken` 取的是「这一份 users 里所有人的头像」，不只是这次
     抽出来的那几张。只做第 2 条是不够的——「我」接住了上一段对话的自拍，新角色又抽到
     同一张，一屏里就出现两张一样的脸（端到端探针第一次就撞上了）。池子里只剩已在用的
     那几张时只能重复，但不空发。
- 随机源可注入（`options.random`），测试里才能断言确定结果；`shuffle` 里对注入值做了夹取，
  返回 1 或负数时也不会抽到 `undefined` 头像。

接线在 `App.tsx` 的 `handleImport`，顺序是：

```
carryOverAvatars → applyPresetsToUsers → 算 carried → sameRoster 判断
  → 名单变了才 drawAvatars → setUsers
```

`avatarPool` 是一个 useMemo：**素材库里 kind 为 avatar 的图** ＋ **用过的头像归档里的图**，
按 dataUrl 去重。只看素材库的话，「明明攒了一堆头像、但都只进了归档」的人抽不出图；
只看归档的话，「从没配过头像、只在素材库传过图」的人同样抽不出来。

三个刻意留下的取舍：

- **「沿用 N 个」在抽之前就算好**，随机补的另说一句「已从头像库随机补上 N 个」。两份混在
  一句话里，用户会以为那些头像是自己续上的、而不是被随机换掉的。
- **时间分隔条不算角色**：`rosterChanged` 只看 `result.users`（解析器不给时间建用户）。
- **批量页没有跟着做**：批量里每一组是另一段对话，按「每次导入都随机」走的话每组都会从
  头像库里抽一轮，整批导出会静默消耗素材库、也没法预期结果。要在批量页也随机，得先决定
  是「整批共用一套」还是「每组各抽一套」，留给用户拍板。

### 二、从用过的头像里直接选取

原来「用过的头像」那一节按名字认人：只有当前对话里正好有同名角色时才给「用上」按钮，
否则只显示「仅保存」——归档里绝大部分条目是点不动的。

新增 `src/components/AvatarPresetDialog.tsx`（配 `AvatarPresetDialog.css`，自带 `--ap-*` 变量，
和素材库弹窗同一个理由：Portal 挂在 body 上，拿不到 `.studio-app` 里的 `--studio-*`）。
每位角色的卡片上多一个「用过的」入口，打开后归档里**任何一张**都能换给**这一位**。要点：

- 弹窗只管「挑一张来用」，改名 / 删除仍留在用户头像管理那一节，免得点错就改到归档；
- 归档一角色一条、用久了能攒几百条，超过 12 条时出现按角色名筛选的输入框，并显示
  「筛出几条 / 共几条」；
- 正在用的那一张标「正在用」（`data-in-use` 静态高亮，不靠 hover）；
- 拆成内外两层与素材库弹窗同理：Portal 默认 `keepMounted=false`，关闭即卸载，筛选词这类
  临时状态不用写 effect 手动清空，下次打开天然是干净的；
- 落地的赋值是 `handleUsePresetFor(userId, preset)`（批量页里是 `applyPresetToJobUser`）。
  它和「还给同名角色」那条并存，没有合并——语义不同：一条是「按名字自动对回来」，
  一条是「明确指定给谁」；
- 换完之后「这个名字 + 这张头像」会被归档记一条（`rememberUserAvatars` 的 effect 照旧跑），
  所以同一张头像换给几个人，归档里就是几条，下次都还能选回来。

### 单测

`user-avatars.test.ts`（新增 6 条、共 13 条）：`sameRoster` 忽略顺序与空白、少一位 / 多一位 /
换一位都算变了、第一次导入（之前为空）算变了；`drawAvatars` 只补没头像的、池子够分时两张
不同；比角色少时转圈复用但只重复必要的次数；空池与脏数据（非 `data:image/` 前缀）原样返回
且不重建数组；避开已在用的那一张（含池子里只剩它时只能重复）；注入的随机源返回 1 / 1.5 / -1
时仍发满。

### 验证边界

纯逻辑由单测兜底，另用一个临时探针按 `handleImport` 的真实顺序跑了四种情形——首次导入 /
同名单改了文字 / 换一段对话 / 换回原来的对话，确认：首次两位角色各拿一张不同的脸；改文字
重新解析后头像一点没变；换对话时「我」接住旧头像、新角色拿到另一张（连跑 5 次没有重复）；
换回原对话时两人从归档里恢复成原来那两张。本机**没有浏览器**：卡片上并列两个小按钮在窄
面板里的排布、选择器网格与筛选框的观感，需要在 Chrome / Edge 里点一遍确认。
