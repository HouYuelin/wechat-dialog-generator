 ...
 * - 逐条设置给 N-1 个间隔（N 条消息），每行一个滑杆，范围 0–3 秒，
 *   既能压到「连着发」的密度（0 秒几乎同时出），也能做出「隔了一会儿才回」的停顿；行上带消息摘要，一眼看出在设哪一条；
...
export function PaceOption({
  ...
  return <>
    ...
    <p className={noteClass}>
      {perMessage
        ? `统一间隔留作默认值——新加进来的消息按它出现，也可以把下面所有间隔一键设成 ${paceSecondsLabel(uniform)} 秒。逐条列表每行是“这一条比上一条晚多久出现”，可调 0–3 秒；开场静置是第一条消息出现前的等待，结尾时长是末条消息之后的留白，都可设成 0 秒。`
