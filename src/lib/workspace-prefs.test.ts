...
test('键序不同也算同一份偏好，避免每渲染一次就写一遍存储', () => {
  ...
  const custom = { ...prefs, playback: { ...prefs.playback, pace: { mode: 'perMessage' as const, paceMs: 1500, gaps: [800, 800], leadInMs: 1200, tailMs: 3000 } } }
  assert.equal(workspacePrefsEqual(prefs, custom), false, '逐条间隔改了也要写进去')
  const sameGaps = { ...prefs, playback: { ...prefs.playback, pace: { mode: 'perMessage' as const, paceMs: prefs.playback.pace.paceMs, gaps: [800, 800], leadInMs: 1200, tailMs: 3000 } } }
  ...
  const tailChanged = { ...prefs, playback: { ...prefs.playback, pace: { ...prefs.playback.pace, tailMs: 0 } } }
  assert.equal(workspacePrefsEqual(prefs, tailChanged), false, '改结尾时长也要写进去')
...
test('节奏偏好：旧的档位折算成毫秒，逐条间隔越界夹回、脏值逐项兜底', () => {
  ...
  assert.deepEqual(normalizeWorkspacePrefs({ playback: { pace: 'fast' } }).playback.pace, { mode: 'uniform', paceMs: 800, gaps: [], leadInMs: 1200, tailMs: 3000 })
